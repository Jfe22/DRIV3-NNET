...
def extra_data(window):
    ...
    print(f'newAccX: {newAccX}')
    print(f'newAccY: {newAccY}')
    print(f'newAccZ: {newAccZ}')
    print(f'newGyroX: {newGyroX}')
    print(f'newGyroY: {newGyroY}')
    print(f'newGyroZ: {newGyroZ}')
    ...
    # ja temos os novos valores de cada sensor
    # podemos retornar los e criar a linhas noutra func ou entao
    # criar a linha aqui mesmo e retornar pronto a concatenar c o resto, idk  
...
def concatenate_dataset(df, window_size, increment, concatenated_rows):
  for i in range(len(df) - window_size + increment):
    ...
    #we will do the dataaugmentation here
    dataaug1_concatenated_sensor_data, dataaug1_label = extra_data(window['sensor_data'])
...
for i in range(1, 11):
  ...
  exit()
  #aqui arranjar maneira de chamar o concatenate 3 vezes
  #a primeira para a tabela normal que originaria a single row
  #e a segunda e terceira parra as novas linhas/tabelas criadas a partir da og
  #ou seja data augmentation
